...
class InsightsScreen(ctk.CTkFrame):
    ...
    def _render_charts(self):
        ...
        for label, data in results.items():
            ...
            # Distribution bar chart (full width — semantic map dropped per user request)
            self._embed_distribution(self._chart_scroll, flat_tax, classifications)
    ...
    def _embed_distribution(self, parent, flat_tax, classifications):
        ...
        n = len(flat_tax)
        fig, ax = plt.subplots(figsize=(10, max(2.5, n * 0.5 + 1)))
        _apply_style(fig, ax)

        # Short labels: just sub-category title (main shown in header)
        labels = [c['title'][:30] for c in flat_tax]
        ...
        ax.barh(y, pcts, height=0.55, color=colors, edgecolor="#0d1117")
        ax.set_yticks(y)
        ...
        for i, (v, p) in enumerate(zip(values, pcts)):
            ax.text(p + 0.8, i, f"{v} ({p}%)", va="center", fontsize=11,
